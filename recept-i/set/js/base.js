...
*
* recept-i is free software: you can redistribute it and/or modify
* it under the terms of the GNU General Public License as published by
* the Free Software Foundation, either version 3 of the License, or
* any later version.
* recept-i is distributed in the hope that it will be useful,
* but WITHOUT ANY WARRANTY; without even the implied warranty of
* MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
* GNU General Public License for more details.
*
* You should have received a copy of the GNU General Public License
* along with recept-i. If not, see <https://www.gnu.org/licenses/>.
*/
...
!function (w, d, u) {
  ...
  var HTML_LOGO = '<div class="logo"><h1>recept</h1><span>i</span></div>';
  ...
  function loadStructure () {
    ...
    addClick(_detach, (e) => {$settings.save(), settings = $settings.get(), toggleClass([_setting, _view], 'hidden'), (module_view == 'recepts') && loadRecepts()})
  ...
  function loadContent () {
    ...
    _search = append(_main, '<div class="module-hedge"><button class="btn btn-accept btn-recepts">Search recipes</button></div>');
    ...
    addClick(only('.btn-recepts', _app), (e) => loadRecepts());
  ...
  function loadRecepts () {
    ...
    _stack = append(_main, '<div class="module-scroller module-recepts module-custom"><div class="module-container"></div></div>');
    ...
    ajax('GET', 'https://api.spoonacular.com/recipes/findByIngredients?' + toQuery(api_data), (result, state, status) => {
      if(state == 4 && status == 200){
        var recepts = JSON.parse(result);
        api_data.ids = recepts.map(a => a.id).join(',');
        ajax('GET', 'https://api.spoonacular.com/recipes/informationBulk?' + toQuery(api_data), (result, state, status) => {
          if(state == 4 && status == 200){
            result = JSON.parse(result);
            result.forEach((recept) => {
              var _recept = append(only('.module-container', _stack),
                '<article class="module-col">\
                  <div class="module-image" style="background-image:url(\'' + recept.image + '\')"></div>\
                  <div class="module-insight module-custom">\
                    <h2>' + recept.title + '</h2>\
                    <div class="module-row">\
                      <span>Time: ' + recept.readyInMinutes + 'min.</span>\
                      <span>health score: ' + recept.healthScore + '</span>\
                    </div>\
                    <p>' + truncateText(recept.summary, 1) + '</p>\
                  </div>\
                </article>');

              addClick(_recept, function (e) {
                loadRecept(this)
              }.bind(recept))
            });
          }
  ...
  /**
   * In this view, the user is shown the chosen recept
   */
  function loadRecept (recept) {
    module_view = 'recept';
    ...
    _stack = append(_main,
      '<div class="module-scroller module-recept module-custom">\
     ...
     */
    if(isArray(recept.analyzedInstructions)){
      recept.analyzedInstructions[0].steps.forEach((step) => {
    ...
    /**
     * When the button is clicked, you get to the recepts view
     */
